...
def plot_masks(n_spines: int, n_dendrites_per_soma: int, n_soma: int,
               ...
               n_itd: Optional[int] = None,
               n_ild: Optional[int] = None,
    ...
    if mode == "alltoall":
        # Bypass build_masks entirely — construct fully dense matrices directly.
        # This is the correct all-to-all baseline: every connection exists.
        input_dim = (n_time or 8) * ((n_freq or 32) + (n_itd or 32) + (n_ild or 32))   # N_TIME * (N_FREQ + N_ITD + N_ILD) = 768
        ...
        aa = int(input_dim * n_spines + n_spines * total_dendrites + total_dendrites * n_soma)
        summary = {
            "config": {
                "input_dim":           input_dim,
                "n_spines":            n_spines,
...
def plot_architectures(n_spines: int, n_dendrites_per_soma: int, n_soma: int,
    ...
    if alltoall:
        # Bypass build_masks entirely for the all-to-all baseline.
        input_dim = 8 * (32 + 32 + 32)
        a1 = int(input_dim * n_spines)
        a2 = int(n_spines * total_dendrites)
        a3 = int(total_dendrites * n_soma)
        aa = a1 + a2 + a3
        reduction = 0.0
        ch_split  = False   # no channel separation in all-to-all
        n_itd_sp  = n_ild_sp = n_spines // 2
        n_itd_de  = n_ild_de = n_dendrites_per_soma // 2
        n_itd_so  = n_ild_so = n_soma // 2
        cfg = {
            "input_dim":           input_dim,
            "n_spines":            n_spines,
            "n_dendrites_per_soma": n_dendrites_per_soma,
    ...
    _box(5, 2.0, 4.2, 0.72, "Output  (all-to-all)",
         "azimuth (deg)  ·  elevation (deg)  ·  distance (m)", TEAL, TEAL_L)
    if ch_split:
...
def verify_figure_masks(
    ...
    expected_files = [
        ...
        f"arch_{mode}_s{n_spines}_dps{n_dendrites_per_soma}_so{n_soma}_{suffix}.png",
        "config.json",
        "history.json",
        "model.keras",
    ]
